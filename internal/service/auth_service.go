...
func NewAuthService(userRepo repository.UserRepo, jwtCfg *config.JWT, tokenBlacklistRepo repository.TokenBlacklistRepo) *AuthService {
	return &AuthService{userRepo: userRepo, jwtCfg: jwtCfg, tokenBlacklistRepo: tokenBlacklistRepo}
}

func (s *AuthService) Register(ctx context.Context, username string, pwd string) error {
	if username == "" || pwd == "" {
...
func (s *AuthService) Login(ctx context.Context, username string, pwd string) (string, error) {
