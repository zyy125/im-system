...
interface UseChatRealtimeParams {
  ...
  currentPublicId: number | undefined
...
export function useChatRealtime({
  ...
  useEffect(() => {
          ...
          return {
            ...
            [message.conversation_id]: [
              ...items,
              {
                ...
                type: message.type,
                event: message.event,
                fromPublicId: message.from_public_id,
                sendTime: message.send_time,
                content: message.content,
                extra: message.extra,
                status: 'sent' as const,
    ...
    const cleanupDelivered = chatWsClient.on('message.delivered', (payload) => {
      ...
      setReceiptState((current) => {
        ...
        const previous = current[key] ?? {
          deliveredByPublicIds: [],
          readByPublicIds: [],
        }
        if (previous.deliveredByPublicIds.includes(event.public_id)) {
          return current
        }
        ...
        return {
          ...
          [key]: {
            ...previous,
            deliveredByPublicIds: [...previous.deliveredByPublicIds, event.public_id],
          },
        }
      })
    })
    ...
    const cleanupRead = chatWsClient.on('message.read', (payload) => {
      ...
      setReceiptState((current) => {
        ...
        if (previous.readByPublicIds.includes(event.public_id)) {
          return current
        }
        ...
        return {
          ...
          [key]: {
            ...previous,
            readByPublicIds: [...previous.readByPublicIds, event.public_id],
          },
        }
      })
    })
    ...
    const cleanupPresence = chatWsClient.on('presence.changed', (payload) => {
      ...
      queryClient.setQueryData(['contacts', 'friends'], (current: unknown) => {
        ...
        return current.map((friend) =>
          friend.publicId === event.public_id
      ...
      queryClient.setQueryData(['chat', 'conversations'], (current: unknown) => {
        ...
        return current.map((conversation) =>
          conversation.peer?.publicId === event.public_id
            ? {
                ...conversation,
                peer: {
                  ...conversation.peer,
  ...
  }, [activeConversationId, conversations, currentPublicId, queryClient])
